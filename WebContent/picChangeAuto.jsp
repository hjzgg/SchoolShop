...
<body>
     <div id="wrapper" spellcheck="false">
        <p>热门推荐</p>
     </div>
	...
	<table id="idSlider2" border="0" cellpadding="0" cellspacing="0">
		<tr>
                      <%
                                ...
                                rt = st.executeQuery("select picName from  salecount order by sale desc");
                                final int cntPic = 6;//要显示的图片的数量
                                int tot = 0;
                                while(rt.next() && ++tot < cntPic){
                                    String picName = rt.getString("picName");
                        %>
                                    <td><a href="./mainChooseFood/showChooseFood.jsp?choosefood=<%=picName%>"><img  src=<%="http://" + ip.getHostAddress() + ":" + port + request.getContextPath() + "/GetPicture?picName=" + picName%> title="<%=picName%>" /></a></td>   
                        <%
                                 }
                        %>
		</tr>
